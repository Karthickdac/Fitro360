...
- **Background workers** (started in `server/index.ts`): `startRetentionSweeper` runs daily — purges access_events older than `eventRetentionMonths` and unmatched_enrolments older than 90 days, logs counts to `activities`. `startEnrolmentSync` runs every 60s — for adapters that implement `listEnrolled`, pulls device-side templates and writes new ones into `unmatched_enrolments` for staff review.
